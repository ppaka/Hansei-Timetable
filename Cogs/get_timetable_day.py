...
class get_timetable_day(commands.Cog):
    ...
    async def request_get(
        ...
        grade: int,
        dddep_name: app_commands.Choice[str],
        class_name: int = 1,
        ...
        if dddep_name == None:
            error_embed = discord.Embed(
                title="으엑, 에러가…", color=0xFF0000, description="학과명이 작성되어지지 않았어요!"
            ).set_footer(text=f"YMD:{ymd} / ppaka")
        ...
        try:
            ...
            await interaction.response.edit_message(
                content=f"알 수 없는 오류가 발생했어요! {str(er_msg)}"
            )
            return
        ...
        if "hisTimetable" not in json_data.keys():
            if json_data["RESULT"]["CODE"] == "INFO-200":
                error_embed = (
                    discord.Embed(
                        title="으엑, 에러가…", color=0xFF0000, description="해당하는 데이터가 없습니다."
                    )
                    .add_field(name="에러코드", value="INFO-200")
                    .set_footer(text=f"YMD:{ymd} / ppaka")
                )
        ...
        if json_data["hisTimetable"][0]["head"][1]["RESULT"]["CODE"] != "INFO-000":
            error_embed = (
                discord.Embed(
                    title="으엑, 에러가…",
                    color=0xFF0000,
                    description="나이스에서 데이터를 불러올 수 없습니다.",
                )
                .add_field(
                    name="에러코드",
                    value=f"{json_data['hisTimetable'][0]['head'][1]['RESULT']['CODE']}",
                )
                .set_footer(text=f"YMD:{ymd} / ppaka")
            )
            await interaction.edit_original_response(embed=error_embed)
        ...
        if "row" not in dict(json_data["hisTimetable"][1]).keys():
            error_embed = discord.Embed(
                title="으엑, 에러가⋯",
                color=0xFF0000,
                description="시간표 데이터를 정상적으로 불러올 수 없습니다. [내부 구문 문제]",
            ).set_footer(text=f"YMD:{ymd} / ppaka")
            await interaction.edit_original_response(embed=error_embed)
            return
        ...
        embed = discord.Embed(
            title=f"{DDDEP_NM} {GRADE}-{CLASS_NM}",
            color=int(color, 0),
            description=f"{day_list[cur_date.weekday()]} {DDDEP_NM} {GRADE}학년 {CLASS_NM}반 시간표를 찾아왔어요!",
        ).set_footer(text=f"YMD:{ymd} / ppaka")
        for e in json_data["hisTimetable"][1]["row"]:
            embed.add_field(
                name=f"{e['PERIO']}교시",
                value=f"{e['ITRT_CNTNT'].replace('* ', '').strip()}",
                inline=False,
            )
...
async def setup(bot: commands.Bot) -> None:
    await bot.add_cog(get_timetable_day(bot))
